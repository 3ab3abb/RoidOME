...
fn router(topic:&str , payload :&str ) -> Result<DeviceEvent , serde_json::Error> { 
...
match router("home/sensors/temeprature" , temphum_raw) { 

        Ok(DeviceEvent::TemperatureHumidity(r)) => println!("{}",r),
        Ok(_) => println!("Unkown Topic ! ") , 
        Err(e) => println!("An Error occured when parsing !") , 
